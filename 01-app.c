...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:{
      ...
      sl_bt_evt_system_boot_t * pPkt = &((evt->data).evt_system_boot);
      app_log_info("BLE system boot event\r\n");
      app_log_info("  Major version: %d\r\n", pPkt->major);
      app_log_info("  Minor version: %d\r\n", pPkt->minor);
      app_log_info("  Patch release: %d\r\n", pPkt->patch);
      app_log_info("  Build number: %d\r\n", pPkt->build);
      app_log_info("  Bootloader: %lu\r\n", pPkt->bootloader);
      app_log_info("  Hardware type: 0x%04X\r\n", pPkt->hw);
      app_log_info("  Hash: 0x%08lX\r\n", pPkt->hash);
    ...
    case sl_bt_evt_connection_opened_id:{
      sl_bt_evt_connection_opened_t *pPkt = &evt->data.evt_connection_opened;
      app_log_info("Connection opened." APP_LOG_NL);
      app_log_info("  Device address: %02X-%02X-%02X-%02X-%02X-%02X\r\n",
                         pPkt->address.addr[5], pPkt->address.addr[4], pPkt->address.addr[3],
                         pPkt->address.addr[2], pPkt->address.addr[1], pPkt->address.addr[0]);
      if (pPkt->address_type == 2)
        {
          app_log_info("  Address type: Resolvable private random address\r\n");
        }
      else
        {
          app_log_info("  Address type: Public/Other\r\n");
        }
      if (pPkt->role == 0)
       {
          app_log_info("  Role: Peripheral\r\n");
       }
     else
       {
         app_log_info("  Role: Central\r\n");
       }
      app_log_info("  Connection handle: %d\r\n", pPkt->connection);
      app_log_info("  Bonding handle: 0x%02X\r\n", pPkt->bonding);
      app_log_info("  Advertiser set: %d\r\n", pPkt->advertiser);
      app_log_info("  Sync handle (for PAwR): 0xFFFF\r\n");
      app_log_info("  Advertising stopped");
      break;
    }
    case sl_bt_evt_connection_parameters_id: {
          sl_bt_evt_connection_parameters_t *pPkt = &evt->data.evt_connection_parameters;
          app_log_info("\x1b[33mConnection parameters update event\x1b[0m\r\n");
          app_log_info("  Connection handle: %d\r\n", pPkt->connection);
          app_log_info("  Connection interval: %d (x1.25 ms)\r\n", pPkt->interval);
          app_log_info("  Connection latency: %d\r\n", pPkt->latency);
          app_log_info("  Connection timeout: %d (x10 ms)\r\n", pPkt->timeout);
          app_log_info("  Security mode: %d\r\n", pPkt->security_mode);
          break;
        }
    case sl_bt_evt_connection_phy_status_id: {
             sl_bt_evt_connection_phy_status_t *pPkt = &evt->data.evt_connection_phy_status;
             app_log_info("  PHY status event\x1b[0m\r\n");
             app_log_info("  Connection handle: %d\r\n", pPkt->connection);
             if(pPkt->phy == 1)
               {
                 app_log_info("  Current PHY: 1M PHY\r\n");
               }
             else
               {
                 app_log_info("  Current PHY: 2M PHY\r\n");
               }
             break;
           }
       case sl_bt_evt_gatt_mtu_exchanged_id: {
             sl_bt_evt_gatt_mtu_exchanged_t *pPkt = &evt->data.evt_gatt_mtu_exchanged;
             app_log_info("  MTU size exchanged event\r\n");
             app_log_info("  Connection handle: %d\r\n", pPkt->connection);
             app_log_info("  MTU size: %d Bytes\r\n", pPkt->mtu);
             break;
           }
    ...
    case sl_bt_evt_connection_closed_id:{
      sl_bt_evt_connection_closed_t *pPkt = &evt->data.evt_connection_closed;
      app_log_info("Connection closed." APP_LOG_NL);
      app_log_info("  Connection handle: %d\r\n", pPkt->connection);
      app_log_info("  Reason: 0x%04X\r\n", pPkt->reason);
    ...
    case sl_bt_evt_gatt_server_attribute_value_id:{
      sl_bt_evt_gatt_server_attribute_value_t *pPkt = &evt->data.evt_gatt_server_attribute_value;
      ...
      if (gattdb_led_control == evt->data.evt_gatt_server_attribute_value.attribute) {
        ...
        app_log_info("Char. led_control written by remote client\r\n");
        app_log_info("  --- Reading Event Data ---\r\n");
        app_log_info("  Connection handle: %d\r\n", pPkt->connection);
        app_log_info("  Attribute handle: 0x%04X\r\n", pPkt->attribute);
        app_log_info("  Attribute opcode: 0x%02X\r\n", pPkt->att_opcode);
        app_log_info("  Offset: %d\r\n", pPkt->offset);

        size_t data_len = pPkt->value.len;
        uint8_t data = pPkt->value.data[0];

        app_log_info("  # bytes: %d\r\n", data_len);
        app_log_info("  Data:  %d\r\n", data);

        app_log_info("  Reading Char. from GATT Server ---\r\n");
        ...
        app_log_info("  Read one byte - value: %d\r\n", data_recv);
        // Toggle LED.
        if (data_recv == 0x00) {
    ...
    case sl_bt_evt_gatt_server_characteristic_status_id:
      if (gattdb_report_button == evt->data.evt_gatt_server_characteristic_status.characteristic) {
          app_log_info("Button CCC changed by remote client\r\n");
